...
<style>
	...
	ul {
		position: relative;
		padding: 0 3rem 0 0;
		background: url(/icons/chevron.svg) calc(100% - 1em) 0.005em no-repeat;
...
<header class:visible="{visible || open}">
	<nav>
		...
		{#if open}
			<div class="modal-background hide-if-desktop" on:click="{() => open = false}" style="opacity:{$opac/100}"></div>
		{/if}
		<div class="segment"><span>Opacity: </span><span>{$opac}</span></div>
		<ul
			class="primary"
			class:open
